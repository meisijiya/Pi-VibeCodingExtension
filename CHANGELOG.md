...
**Bug 追踪（渐进式加载）：**
- `vibe_bug` 工具 + `/vibe-bug` 命令 — LLM 发现 bug 时自动标记（记录文件、行、引入提交）
- `vibe_bug_fix` 工具 + `/vibe-bug-fix` 命令 — 修复后标记状态
- `vibe_bug_info` 工具 — LLM 按需加载 bug 详情（不全量注入）
- 渐进式注入：上下文只显示文件名 + 状态（🔴/✅），详情按需读取
